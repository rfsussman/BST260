...
In the terminal, enter `quarto render HW0.qmd --to pdf`. To hide the R code, type `execute: echo: false` in the Quarto file header. Alternatively, in the RStudio console, enter `rmarkdown::render("HW0.qmd", "pdf_document")`. To hide the R code, type `echo = FALSE` in each bracket that initializes a code chunk.
