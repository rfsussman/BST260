...
A given quadratic equation can have 0, 1, or 2 solutions. A quadratic equation has no real solutions when the curve does not intersect with the line y=0, one solution when the curve intersects with the line y=0 once (at the vertex of the parabola), and two solutions when the curve intersects with the line y=0 twice. The number of solutions is determined by the *discriminant* of the formula, or $b^{2}-4ac$. When the discriminant is greater than 0, there are two solutions, when the discriminant is equal to 0, there is one solution, and when the discriminant is less than 0, there are no solutions (since the square root cannot be negative).
